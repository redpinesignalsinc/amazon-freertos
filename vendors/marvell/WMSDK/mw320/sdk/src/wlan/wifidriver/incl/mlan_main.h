...
typedef struct {
#ifdef STA_SUPPORT
	mlan_status (*wlan_11d_prepare_dnld_domain_info_cmd_p)
			(mlan_private * pmpriv);
	mlan_status (*wlan_11d_create_dnld_countryinfo_p)
			(mlan_private * pmpriv, t_u8 band);
	mlan_status (*wlan_11d_parse_dnld_countryinfo_p)
			(mlan_private * pmpriv, BSSDescriptor_t * pbss_desc);
#endif
} wlan_11d_fn_t;
...
typedef struct {
	mlan_status (*wlan_11d_cfg_ioctl_p)
			(IN mlan_private * pmpriv,
			IN pmlan_ioctl_req pioctl_req);
	mlan_status (*wlan_11d_cfg_domain_info_p)
			(IN pmlan_adapter pmadapter,
			IN mlan_ioctl_req * pioctl_req);
	mlan_status (*wlan_cmd_802_11d_domain_info_p)
			(mlan_private * pmpriv,
			HostCmd_DS_COMMAND * pcmd, t_u16 cmd_action);
	mlan_status (*wlan_11d_handle_uap_domain_info_p)
			(mlan_private * pmpriv, t_u8 band,
...
t_u8 wlan_11d_get_scan_type(mlan_private * pmpriv, t_u8 band, t_u8 chan,
		parsed_region_chan_11d_t * parsed_region_chan);
