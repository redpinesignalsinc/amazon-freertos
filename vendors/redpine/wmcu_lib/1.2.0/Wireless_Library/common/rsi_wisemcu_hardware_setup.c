...
/**
 * @fn        void RSI_WISEMCU_HardwareSetup(void)
 * @brief	    This is used to configure the default hardware configuration required for 'WiSeMCU' mode.
 * @return    none.
 * @par note  This function must be called in main before using any power save related configurations in applications.
 */
void RSI_WISEMCU_HardwareSetup(void)
{
	...
	/* Disabling 40MHz Clocks */
	RSI_ULPSS_DisableRefClks( MCU_ULP_40MHZ_CLK_EN ); 			
	/* Disabling RC_32KHZ Clocks */
	...
	/* Configure PMU Start-up Time to be used on Wakeup */	
	RSI_PS_PmuGoodTimeDurationConfig(PMU_GOOD_TIME);		
	/* Configure XTAL Start-up Time to be used on Wakeup */
...
/**
 * @fn        void RSI_WISEMCU_TriggerSleep(SLEEP_TYPE_T sleepType , uint8_t  lf_clk_mode    , 
 ...
 *                              \n '0' : ' \ref DISABLE_LF_MODE' Normal mode of operation , recommended in most of the applications.
 ...
 * @param[in] mode            : Following are the possible parameters for this parameter.
		...
		uint32_t vector_offset , uint32_t mode)
{
	...
	//! Turn on the ULPSS rams, domains and retain ULPSS rams 
  ...
  //! Peripherals which are needed on wakeup (without ram retention) needs to be powered up before going to sleep
  if((mode == RSI_WAKEUP_WITH_OUT_RETENTION)||(mode == RSI_WAKEUP_WO_RETENTION_WO_ULPSS_RAM))
  {
	...
	//! Indicating M4 is Inactive
	P2P_STATUS_REG &= ~M4_is_active;	
	DEBUGOUT("\n Waiting for TA allow M4 to sleep\n");

	if((P2P_STATUS_REG & TA_wakeup_M4) 
			||(P2P_STATUS_REG & M4_wakeup_TA)
			||(rsi_driver_cb->m4_tx_q.pending_pkt_count) 
			||(rsi_driver_cb->common_tx_q.pending_pkt_count) 
			||(rsi_driver_cb->wlan_tx_q.pending_pkt_count)
#if (defined RSI_BT_ENABLE || defined RSI_BLE_ENABLE)
			||(rsi_driver_cb->bt_single_tx_q->pending_pkt_count)
			||(rsi_driver_cb->bt_common_tx_q.pending_pkt_count)
#endif	
#ifdef RSI_BT_ENABLE
	...
	//! Moving M4 SOC clock to ULP reference clock before going to powersave
	RSI_CLK_M4SocClkConfig(M4CLK,M4_ULPREFCLK,0);

	//! Taking backup before going to powersave
	...
	//! Configuring the sleep parameters required by bootloader upon wakeup
	...
	//! Systick configuration upon wakeup
	SysTick_Config(SystemCoreClock/1000);

	//! Indicating M4 is active
	P2P_STATUS_REG |= M4_is_active;

	//! Restoring the values from backup
...
/**
 * @fn        void RSI_WISEMCU_ConfigRamRetention(uint32_t rams_in_use, uint32_t rams_retention_during_sleep)
 * @brief	    This is used to configure the default hardware configuration required for 'WiSeMCU' mode.
 * @param[in] rams_power_gate             : Rams to be powered functioanlly , Rest of the RAM banks will be power gates.
 *                                          \n Following macros to be used for this parameter
 ...
 * @param[in] rams_retention_during_sleep : Configures the RAM retentions to the hardware , so that particular RAM banks are retained during sleep 
 *                                          \n Following macros to be used for this parameter
 *                                          \n WISEMCU_RETAIN_DEFAULT_RAM_DURING_SLEEP : Selects the RAM Retentions controls automatically by API based on 'rams_power_gate' value passed by user
...
void RSI_WISEMCU_ConfigRamRetention(uint32_t rams_in_use, uint32_t rams_retention_during_sleep)
{
	...
	RSI_PS_ClrRamRetention(M4ULP_RAM16K_RETENTION_MODE_EN | TA_RAM_RETENTION_MODE_EN | M4ULP_RAM_RETENTION_MODE_EN | M4SS_RAM_RETENTION_MODE_EN);     
	...
	{
		/*If none of the Banks are powered on so clear all retention controlls*/
		...
		if((rams_in_use & WISEMCU_48KB_RAM_IN_USE) || (rams_in_use & WISEMCU_112KB_RAM_IN_USE) || (rams_in_use & WISEMCU_128KB_RAM_IN_USE)){
			RSI_PS_SetRamRetention(M4ULP_RAM_RETENTION_MODE_EN);     
		}   
		/*Set the M4SS 64KB RAM retention */   
		if((rams_in_use & WISEMCU_144KB_RAM_IN_USE) || (rams_in_use & WISEMCU_176KB_RAM_IN_USE) || (rams_in_use & WISEMCU_192KB_RAM_IN_USE)){
			RSI_PS_SetRamRetention(M4SS_RAM_RETENTION_MODE_EN);     
		}   
		/*Set the M4SS 64KB RAM retention */   
		if((rams_in_use & WISEMCU_208KB_RAM_IN_USE) || (rams_in_use & WISEMCU_240KB_RAM_IN_USE) || (rams_in_use & WISEMCU_320KB_RAM_IN_USE) || (rams_in_use & WISEMCU_384KB_RAM_IN_USE)){
