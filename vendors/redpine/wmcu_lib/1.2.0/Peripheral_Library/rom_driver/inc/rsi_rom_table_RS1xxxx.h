 ...
 *
 * @brief This files contains functions tables for ROM entry 
 *
 * @section Description
 * This files contains functions tables for ROM entry 
 *
 *
 */
...
typedef PRE_PACK struct POST_PACK
	...
	error_t (*clk_soc_pll_set_freq_div)(M4Clk_Type *pCLK , boolean_t clk_en,uint16_t divFactor,uint16_t nFactor,uint16_t mFactor,uint16_t fcwF,uint16_t dcoFixSel,uint16_t ldoProg);
	...
	error_t (*clk_i2s_pll_set_freq_div)(M4Clk_Type *pCLK,uint16_t u16DivFactor1,uint16_t u16DivFactor2,uint16_t nFactor,uint16_t mFactor, uint16_t fcwF );
	...
	error_t (*clk_intf_pll_set_freq_div)(M4Clk_Type *pCLK , boolean_t clk_en,uint16_t divFactor,uint16_t nFactor,uint16_t mFactor,uint16_t fcwF,uint16_t dcoFixSel,uint16_t ldoProg);
	...
	error_t (*clk_m4_soc_clk_config)(M4Clk_Type *pCLK ,
	...
	error_t (*clk_sd_mem_clk_config)(M4Clk_Type *pCLK ,
			...
			uint32_t divFactor);
	error_t (*clk_ct_clk_config)(M4Clk_Type *pCLK ,
			CT_CLK_SRC_SEL_T clkSource ,
			uint32_t divFactor,CLK_ENABLE_T clkType);
	error_t (*clk_cci_clk_config)(M4Clk_Type *pCLK ,
			CCI_CLK_SRC_SEL_T clkSource ,
			uint32_t divFactor,CLK_ENABLE_T clkType);
	error_t (*clk_i2s_clk_config)(M4Clk_Type *pCLK ,
			I2S_CLK_SRC_SEL_T clkSource ,
			uint32_t divFactor);
	error_t (*clk_mcu_clk_cut_config)(M4Clk_Type *pCLK ,
			MCU_CLKOUT_SRC_SEL_T   clkSource  ,
			uint32_t divFactor);
	error_t (*clk_can_clk_config)(M4Clk_Type *pCLK ,	uint32_t divFactor,CLK_ENABLE_T clkType);
	error_t (*clk_ethernet_clk_config)(M4Clk_Type *pCLK ,boolean_t swalloEn ,ETHERNET_CLK_SRC_SEL_T clkSource,uint32_t divFactor);
...
typedef PRE_PACK struct POST_PACK {
	...
	void (*qspi_status_reg_write)(qspi_reg_t *qspi_reg,	uint32_t write_value, spi_config_t *spi_config,uint32_t wr_reg_delay_ms);
	uint32_t (*qspi_flash_reg_read)(qspi_reg_t *qspi_reg, uint8_t reg_read_cmd, uint32_t cs_no,  spi_config_t *spi_config);
	void (*qspi_flash_reg_write)(qspi_reg_t *qspi_reg,		uint32_t reg_write_cmd,		uint32_t reg_write_value,		uint32_t cs_no, uint32_t wr_reg_delay_ms);
	void (*qspi_set_flash_mode)(qspi_reg_t *qspi_reg,	uint32_t data_mode,	uint32_t cs_no,	uint32_t ddr_en,	uint32_t flash_type);
	void (*qspi_config_qflash4_read)(qspi_reg_t *qspi_reg,spi_config_t *spi_config,	uint32_t addr);
	void (*qspi_manual_read)(qspi_reg_t *qspi_reg,spi_config_t *spi_config,uint32_t addr,uint8_t *data,uint32_t hsize,	uint32_t len_in_bytes, uint32_t manual_udma_read, void *udmaHandle, void *gpdmaHandle);
	void (*qspi_auto_init)(qspi_reg_t *qspi_reg,spi_config_t *spi_config);
	void (*qspi_auto_read)(uint32_t cs_no, uint32_t addr, uint8_t *data, uint32_t hsize, uint32_t len_in_bytes, spi_config_t *spi_config, uint32_t dma_flags);
	...
	uint32_t (*qspi_spi_write)(qspi_reg_t *qspi_reg,spi_config_t *spi_config,uint32_t write_cmd,uint32_t addr,uint8_t  *data,uint32_t len_in_bytes,uint16_t page_size,	uint32_t hsize,uint32_t dis_hw_ctrl,uint32_t wr_reg_delay_ms,uint32_t check_en,uint32_t udma_enable,void *udmaHandle,void *gpdmaHandle);
	void (*qspi_spi_read)(qspi_reg_t *qspi_reg,	spi_config_t *spi_config,	uint32_t addr,uint8_t  *data,	uint32_t  hsize,	uint32_t len_in_bytes, uint32_t manual_udma_read, void *udmaHandle,  void *gpdmaHandle);
	void (*qspi_usleep)(uint32_t delay_us);    //!< function ptr for halting processor for delay (us) specified
	void (*qspi_write_block_protect)(qspi_reg_t *qspi_reg,	uint32_t protect,	uint32_t cs_no,	uint32_t num_prot_bytes,	uint32_t wr_reg_delay_ms);
...
typedef PRE_PACK struct POST_PACK {
	...
	void (*RSI_GotoSleepWithRetention)(uint32_t stack_address, uint32_t jump_cb_address, uint32_t vector_offset,uint32_t mode);
...
typedef PRE_PACK struct POST_PACK
	...
	error_t (*ulpss_ulp_i2s_clk_config)(ULPClk_Type *pULPCLK  ,
	...
	error_t (*ulpss_aux_clk_config)(ULPClk_Type *pULPCLK ,
			CLK_ENABLE_T clkType,
	...
	error_t (*ulpss_touch_clk_config)(ULPClk_Type *pULPCLK ,
...
typedef PRE_PACK struct POST_PACK 
	...
	uint32_t (* udma_get_channel_transfer_mode)(RSI_UDMA_HANDLE_T pHandle, 
	...
	error_t (* udma_set_channel_scatter_gather_transfer)( RSI_UDMA_HANDLE_T pHandle, 
			uint8_t dmaCh, uint32_t taskCount,
...
typedef PRE_PACK struct POST_PACK 
	...
	error_t (* mcpwm_set_time_period)( RSI_MCPWM_T *pMCPWM, uint8_t chnlNum, 
			uint16_t period, uint16_t initVal);

	void (* mcpwm_special_event_trigger_config)( RSI_MCPWM_T *pMCPWM, boolean_t   svtDir,
			RSI_MCPWM_SVT_CONFIG_T *pMCPWMSVTConfig);

	error_t (* mcpwm_dead_time_value_set)(RSI_MCPWM_T *pMCPWM , RSI_MCPWM_DT_CONFIG_T *pMCPWMDeadTimeConfig,
			uint8_t chnlNum);

	error_t (* mcpwm_period_control_config)( RSI_MCPWM_T *pMCPWM, uint32_t postScale,
			uint32_t preScale, uint8_t chnlNum );

	error_t (* mcpwm_fault_avalue_set)( RSI_MCPWM_T *pMCPWM,uint8_t pwmOutput,
			uint8_t value );

	error_t (* mcpwm_fault_bvalue_set)( RSI_MCPWM_T *pMCPWM,uint8_t pwmOutput,
			uint8_t value );

	error_t (* mcpwm_set_base_timer_mode)(RSI_MCPWM_T *pMCPWM,uint8_t mode,	
			uint8_t chnlNum);

	error_t (* mcpwm_set_output_mode)( RSI_MCPWM_T *pMCPWM, boolean_t mode, 
			uint8_t chnlNum );

	void (* mcpwm_set_output_polarity)( RSI_MCPWM_T *pMCPWM, 
			boolean_t polL,boolean_t polH);

	void (* mcpwm_interrupt_handler)( RSI_MCPWM_T *pMCPWM, 
...
typedef PRE_PACK struct POST_PACK 
	...
	RSI_GPDMA_HANDLE_T (* gpdma_init)(void *mem,
			const RSI_GPDMA_INIT_T *pInit);

	void (* gpdma_register_callback)(RSI_GPDMA_HANDLE_T pHandle,
			uint32_t cbIndex, void *pCB);

	error_t (* gpdma_abort_channel)(RSI_GPDMA_HANDLE_T pHandle,uint8_t dmaCh);

	error_t (* gpdma_setup_channel)(RSI_GPDMA_HANDLE_T pHandle,
	...
	error_t (* gpdma_setup_channelTransfer)(RSI_GPDMA_HANDLE_T pHandle, uint8_t dmaCh,
			RSI_GPDMA_DESC_T *pDesc);
...
#define ROMAPI_RPDMA_API 	          ((RSI_ROM_API)->pRPDMAROM)
