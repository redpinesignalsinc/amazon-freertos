 ...
 *
 * @brief This files contains functions prototypes related to m4ss clks
 *
 * @section Description
 * This file contains the list of function prototypes for the m4ss clks and low level function definitions
...
{
#if defined(ROMDRIVER_PRESENT)
	return ROMAPI_M4SS_CLK_API->clk_soc_pll_set_freq_div(pCLK,clk_en,divFactor,nFactor ,mFactor,fCwf,dcofixsel,ldoprog);
#else
 ...
 * @return 		  returns zero \ref RSI_OK  on success ,on failure return error code.
 */
STATIC INLINE error_t RSI_CLK_I2sPllSetFreqDiv(M4Clk_Type *pCLK,uint16_t u16DivFactor1,uint16_t u16DivFactor2,uint16_t nFactor,uint16_t mFactor, uint16_t fcwF)
{
#if defined(ROMDRIVER_PRESENT)
 ...
 *              \n For >=251Mhz ---> ldo_prog =5 and dco_fix_sel=2
 */
STATIC INLINE error_t  RSI_CLK_IntfPllSetFreqDiv(M4Clk_Type *pCLK , boolean_t clk_en,uint16_t divFactor,uint16_t nFactor,uint16_t mFactor,uint16_t fcwF,uint16_t dcoFixSel,uint16_t ldoProg)
{
#if defined(ROMDRIVER_PRESENT)
	return ROMAPI_M4SS_CLK_API->clk_intf_pll_set_freq_div(pCLK,clk_en,divFactor,nFactor ,mFactor,fcwF,dcoFixSel,ldoProg);
#else
 ...
 */
STATIC INLINE error_t RSI_CLK_QspiClkConfig(M4Clk_Type *pCLK ,QSPI_CLK_SRC_SEL_T clkSource,boolean_t swalloEn,
		boolean_t OddDivEn,uint32_t divFactor)
{
#if defined(ROMDRIVER_PRESENT)
 ...
 * @return 		  returns zero \ref RSI_OK  on success ,on failure return error code.
 */
STATIC INLINE error_t RSI_CLK_SsiMstClkConfig(M4Clk_Type *pCLK ,CLK_ENABLE_T clkType  ,SSI_MST_CLK_SRC_SEL_T clkSource ,
		uint32_t   divFactor)
{
#if defined(ROMDRIVER_PRESENT)
 ...
 * @return 		  returns zero \ref RSI_OK  on success ,on failure return error code.
 */
STATIC INLINE error_t RSI_CLK_SdMemClkConfig(M4Clk_Type *pCLK ,boolean_t swalloEn ,SDMEM_CLK_SRC_SEL_T clkSource ,
		uint32_t divFactor)
{
#if defined(ROMDRIVER_PRESENT)
 ...
 * @return 		  returns zero \ref RSI_OK  on success ,on failure return error code.
 */
STATIC INLINE error_t RSI_CLK_CciClkConfig(M4Clk_Type *pCLK ,CCI_CLK_SRC_SEL_T clkSource ,uint32_t divFactor,
		CLK_ENABLE_T clkType)
{
#if defined(ROMDRIVER_PRESENT)
 ...
 * @return 		  returns zero \ref RSI_OK  on success ,on failure return error code.
 */
STATIC INLINE error_t RSI_CLK_EthernetClkConfig(M4Clk_Type *pCLK  ,boolean_t swalloEn ,ETHERNET_CLK_SRC_SEL_T clkSource,
		uint32_t divFactor)
{
#if defined(ROMDRIVER_PRESENT)
 ...
 * @return 		  returns zero \ref RSI_OK  on success ,on failure return error code.
 */
STATIC INLINE error_t RSI_CLK_QspiClkDiv(M4Clk_Type *pCLK , boolean_t u8SwallowEn , boolean_t u8OddDivEn , uint32_t divFactor)
{
#if defined(ROMDRIVER_PRESENT)
 ...
 * @return 		  returns zero \ref RSI_OK  on success ,on failure return error code.
 */
STATIC INLINE error_t RSI_CLK_UsartClkDiv(M4Clk_Type *pCLK , EN_USART_T EN_USART_T , uint8_t u8FracDivEn, uint32_t divFactor)
{
#if defined(ROMDRIVER_PRESENT)
...
{
  //Fixed usart disable issue ,and moved function from ROM to RAM
//#if defined(ROMDRIVER_PRESENT)
