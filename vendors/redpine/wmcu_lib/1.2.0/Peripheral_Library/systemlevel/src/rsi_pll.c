 ...
 *
 * @brief This files contains functions  related to PLL  peripheral
 *
 * @par Description
 * This file contains the list of function for the PLL and low level function definitions
...
static uint32_t RSI_CLK_CheckPresent(M4Clk_Type *pCLK  ,CLK_PRESENT_T clkPresent);
...
error_t clk_set_soc_pll_freq(M4Clk_Type *pCLK,uint32_t socPllFreq,uint32_t pllRefClk)
{
  ...
  //system_clocks.soc_pll_clock 		= socPllFreq;
	...
	if(!(pCLK != NULL) && (socPllFreq >= SOC_PLL_MIN_FREQUECY) &&
			(socPllFreq <= SOC_PLL_MAX_FREQUECY ))
	{
  ...
  // Programming enable for bypass_lock_pll and count as 0x100
...
error_t clk_i2s_pll_clk_set(M4Clk_Type *pCLK)
{
	...
	while((pCLK->PLL_STAT_REG_b.I2SPLL_LOCK) != 1); /* checking for pll lck */
	return RSI_OK;
}
...
error_t clk_set_intf_pll_freq(M4Clk_Type *pCLK,uint32_t intfPllFreq,uint32_t pllRefClk)
{
	...
	/*Parameter validation */
	if(!(pCLK != NULL) && (intfPllFreq >= INTF_PLL_MIN_FREQUECY) &&
			(intfPllFreq <= INTF_PLL_MAX_FREQUECY ))
	{
		return INVALID_PARAMETERS;
	}
  ...
  // Intf PLL bypass enable to avoid clock gating
...
error_t clk_intf_pll_clk_set(M4Clk_Type *pCLK)
{
	...
	while((pCLK->PLL_STAT_REG_b.INTFPLL_LOCK)!= 1); // checking for pll lck
	return RSI_OK;
}
...
/**
 * @fn          static uint32_t RSI_CLK_CheckPresent(M4Clk_Type *pCLK  ,CLK_PRESENT_T clkPresent)
		...
		uint32_t divFactor)
{
	...
	if(errorCode == RSI_OK)
	{
	  ...
	  Before enabling this make sure that qspi_clk_onehot_enable is 1’b0 to enable glitch free switching*/
		...
		uint32_t divFactor)
{
	...
	if(enUsart == USART1)
	{	
		clk_peripheral_clk_disable(pCLK,USART1_CLK);
	}
	else
	{
		clk_peripheral_clk_disable(pCLK,USART2_CLK);
	}	
	/*Select clock MUX*/
	switch(clkSource)
	{
	...
	if(enUsart == USART1)
	{
		clk_peripheral_clk_enable(pCLK,USART1_CLK,clkType);
	}
	else
	{
		clk_peripheral_clk_enable(pCLK,USART2_CLK,clkType);
	}	
	return RSI_OK;
}
...
error_t clk_usb_clk_config(M4Clk_Type *pCLK ,USB_CLK_SRC_SEL_T clkSource ,uint16_t divFactor)
{
	...
	if(!(pCLK != NULL) && (divFactor >= USB_MAX_CLK_DIVISION_FACTOR) &&
			(divFactor <= USB_MIN_CLK_DIVISION_FACTOR ))
	{
		return INVALID_PARAMETERS;
	}
	/*clock Disable */
