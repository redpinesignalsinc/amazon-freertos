 ...
 *
 * @brief This files contains functions related to ULPSS clock
 *
 * @section Description
 * This file contains the list of functions for the ULPSS clock and low level function definitions
		...
		cdDelay   delayFn)
{
	...
	if(!(pULPCLK != NULL)  &&
			(divFactor <= ULP_PROC_MAX_DIVISOIN_FACTOR ) &&
			(clkSource <= ULP_PROC_MAX_SEL))
	{
		return INVALID_PARAMETERS;
	}

//	/*clock disable */
//	pULPCLK->ULP_TA_CLK_GEN_REG_b.ULP_PROC_CLK_EN_b = 0;
	...
	case ULP_PROC_ULP_32KHZ_XTAL_CLK:
		/*Enable clock*/
    RSI_SetRegSpiDivision(0U);
    ulpss_enable_ref_clks(MCU_ULP_32KHZ_XTAL_CLK_EN, ULP_PROCESSOR_CLK,delayFn);		pULPCLK->ULP_TA_CLK_GEN_REG_b.ULP_PROC_CLK_SEL = clkSource;
		...
		uint16_t               divFactor)
{
	/*Parameter validation */
	if(!(pULPCLK != NULL) && 
			(divFactor <= ULP_SSI_MAX_DIVISION_FACTOR ) &&
			(clkSource <= ULP_SSI_MAX_SEL))
	{
		return INVALID_PARAMETERS;
	}
		...
		uint16_t              divFactor)
{
	/*Parameter validation */
	if(!(pULPCLK != NULL) && 
			(divFactor <= ULP_I2S_MAX_DIVISION_FACTOR ) &&
			(clkSource <= ULP_I2S_MAX_SEL))
	{
		return INVALID_PARAMETERS;
	}
...
{
	/*Parameter validation */
	if(!(pULPCLK != NULL) && (divFactor <= ULP_UART_MAX_DIVISION_FACTOR ) &&
			(clkSource <= ULP_UART_MAX_SEL) )
	{
		return INVALID_PARAMETERS;
	}
		...
		uint16_t divFactor)
{
	/*Parameter validation */
	if(!((pULPCLK != NULL) &&
			(divFactor <= ULP_VAD_MAX_DIVISION_FACTOR ) &&
			(clkSource <= ULP_VAD_MAX_SEL) && (FclkSource <= ULP_VAD_FCLK_MAX_SEL)))
	{
		return INVALID_PARAMETERS;
	}
...
{
	/*Parameter validation */
	if(!(pULPCLK != NULL)  &&  (divFactor <= ULP_TOUCH_MAX_DIVISION_FACTOR ) &&
			(clkSource <= ULP_TOUCH_MAX_SEL) )
	{
		return INVALID_PARAMETERS;
	}
...
error_t ulpss_slp_sensor_clk_config(ULPClk_Type *pULPCLK , boolean_t clkEnable ,uint32_t divFactor)
{
	/*Parameter validation */
	if(!(pULPCLK != NULL) &&
			(divFactor <= ULP_SLP_SENSOR_MAX_DIVISION_FACTOR ))
	{
		return INVALID_PARAMETERS;
	}
	if(clkEnable)
