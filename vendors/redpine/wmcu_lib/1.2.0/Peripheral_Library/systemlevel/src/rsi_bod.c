...
float RSI_BOD_SoftTriggerGetBatteryStatus(void) 
{
	...
	ULP_SPI_MEM_MAP(BOD_VBATT_STATUS_REG) |= BOD_PWRGATE_EN_N_ULP_BUTTON_CALIB|( CHECK_VBATT_STATUS) ; //set bit 15 and bit 7 */
 ...
 if(vbatt_status_trim_efuse[2])
 {
	...
	vbatt = BOD_VBATT_COMPUT_FIRST_ELEMENT * (BOD_VBATT_COMPUT_SECOND_ELEMENT + batt_status) / (BOD_VBATT_COMPUT_SECOND_ELEMENT+ batt_status_supply) ;
 }
else
{
/* converts the battery status to vbatt voltage value*/	
...
void RSI_BOD_Enable(uint8_t enable,float vbatt_threshold)
{
	...
	if (enable==1)
	{
		...
		threshold_f 	=	((vbatt_threshold /  BOD_VBATT_COMPUT_FIRST_ELEMENT)*(BOD_VBATT_COMPUT_SECOND_ELEMENT + BOD_VBATT_COMPUT_THIRD_ELEMENT) )- BOD_VBATT_COMPUT_SECOND_ELEMENT ;
		/* Converts the thershold value to integer value */
		threshold_i = (float)((vbatt_threshold /  BOD_VBATT_COMPUT_FIRST_ELEMENT)*(BOD_VBATT_COMPUT_SECOND_ELEMENT + BOD_VBATT_COMPUT_THIRD_ELEMENT) )- BOD_VBATT_COMPUT_SECOND_ELEMENT ;
