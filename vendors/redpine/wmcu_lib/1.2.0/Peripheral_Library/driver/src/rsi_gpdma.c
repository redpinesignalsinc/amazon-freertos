 ...
 *
 * @brief This files contains function wrappers related to GPDMA.
 *
 * @par Description
 * This file contains the list of function wrappers for the GPDMA .
 *
 */
#include "rsi_chip.h"
...
error_t gpdma_interrupt_clear( RSI_GPDMA_HANDLE_T pHandle ,
...
uint32_t gpdma_interrupt_status( RSI_GPDMA_HANDLE_T pHandle ,
                                    RSI_GPDMA_CHA_CFG_T *pCfg )
{
	GPDMA_DATACONTEXT_T *pDrv = (GPDMA_DATACONTEXT_T *) pHandle;
...
uint32_t gpdma_get_error_status( RSI_GPDMA_HANDLE_T pHandle ,
                                   RSI_GPDMA_CHA_CFG_T *pCfg )
{
	GPDMA_DATACONTEXT_T *pDrv = (GPDMA_DATACONTEXT_T *) pHandle;
...
error_t gpdma_error_status_clear( RSI_GPDMA_HANDLE_T pHandle, 
                                    RSI_GPDMA_CHA_CFG_T *pCfg )
{ 
	GPDMA_DATACONTEXT_T *pDrv = (GPDMA_DATACONTEXT_T *) pHandle;
...
static uint8_t RSI_GPDMA_GetChannelActivity( RSI_GPDMA_HANDLE_T pHandle,
                                             uint8_t dmaCh )
{
	GPDMA_DATACONTEXT_T *pDrv = (GPDMA_DATACONTEXT_T *) pHandle;
...
error_t gpdma_interrupt_enable( RSI_GPDMA_HANDLE_T pHandle, 
                                   RSI_GPDMA_CHA_CFG_T *pCfg )                           
{
	uint32_t maskReg=0;
	GPDMA_DATACONTEXT_T *pDrv = (GPDMA_DATACONTEXT_T *) pHandle;
...
error_t gpdma_interrupt_disable( RSI_GPDMA_HANDLE_T pHandle, 
                                    RSI_GPDMA_CHA_CFG_T *pCfg )                           
{
	uint32_t maskReg=0;
	GPDMA_DATACONTEXT_T *pDrv = (GPDMA_DATACONTEXT_T *) pHandle;
...
{ 
	...
	if((pXferCfg->chnlCtrlConfig.transType == MEMORY_MEMORY)||
			(pXferCfg->chnlCtrlConfig.transType == MEMORY_PERIPHERAL)||
	...
	/* Set flow controller */
	if((pXferCfg->chnlCtrlConfig.dmaFlwCtrl == DMA_FLW_CTRL)||
			(pXferCfg->chnlCtrlConfig.dmaFlwCtrl == DST_PERI_CTRL)||
                                        ...
                                        RSI_GPDMA_DESC_T *pDesc)
{
	...
	if(dmaCh <= GPDMA_CHNL7)
	{
		...
		/* Assigns transfer mode */
		if((pDesc->chnlCtrlConfig.transType == MEMORY_MEMORY)||
				(pDesc->chnlCtrlConfig.transType == MEMORY_PERIPHERAL)||
		...
		/* Set flow controller */
		if((pDesc->chnlCtrlConfig.dmaFlwCtrl == DMA_FLW_CTRL)||
				(pDesc->chnlCtrlConfig.dmaFlwCtrl == DST_PERI_CTRL)||
		...
		{
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].CHANNEL_CTRL_REG_CHNL_b.MSTR_IF_FETCH_SEL = pDesc->chnlCtrlConfig.mastrIfFetchSel;  
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		{
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].CHANNEL_CTRL_REG_CHNL_b.MSTR_IF_SEND_SEL  = pDesc->chnlCtrlConfig.mastrIfSendSel; 
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		{
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].CHANNEL_CTRL_REG_CHNL_b.LINK_LIST_MSTR_SEL = pDesc->chnlCtrlConfig.linkListMstrSel;
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		pDrv->baseC->CHANNEL_CONFIG[dmaCh].CHANNEL_CTRL_REG_CHNL_b.SRC_ADDR_CONTIGUOUS = pDesc->chnlCtrlConfig.srcAddContiguous;
		pDrv->baseC->CHANNEL_CONFIG[dmaCh].CHANNEL_CTRL_REG_CHNL_b.DEST_ADDR_CONTIGUOUS = pDesc->chnlCtrlConfig.dstAddContiguous;
		...
		{		
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].MISC_CHANNEL_CTRL_REG_CHNL_b.AHB_BURST_SIZE = pDesc->miscChnlCtrlConfig.ahbBurstSize;
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		{
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].MISC_CHANNEL_CTRL_REG_CHNL_b.SRC_DATA_BURST = pDesc->miscChnlCtrlConfig.srcDataBurst;
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		{
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].MISC_CHANNEL_CTRL_REG_CHNL_b.DEST_DATA_BURST = pDesc->miscChnlCtrlConfig.destDataBurst;
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		{
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].MISC_CHANNEL_CTRL_REG_CHNL_b.DEST_CHNL_ID = pDesc->miscChnlCtrlConfig.destChannelId;
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		{
			pDrv->baseC->CHANNEL_CONFIG[dmaCh].MISC_CHANNEL_CTRL_REG_CHNL_b.SRC_CHNL_ID = pDesc->miscChnlCtrlConfig.srcChannelId;
		}
		else
		{
			return ERROR_GPDMA_INVALID_ARG;
		...
		/* Memory fill enable */	
		pDrv->baseC->CHANNEL_CONFIG[dmaCh].MISC_CHANNEL_CTRL_REG_CHNL_b.MEM_FILL_ENABLE = pDesc->miscChnlCtrlConfig.memoryFillEn;
		/* Memory fill with either 0 or 1 */
		pDrv->baseC->CHANNEL_CONFIG[dmaCh].MISC_CHANNEL_CTRL_REG_CHNL_b.MEM_ONE_FILL = pDesc->miscChnlCtrlConfig.memoryOneFill;
