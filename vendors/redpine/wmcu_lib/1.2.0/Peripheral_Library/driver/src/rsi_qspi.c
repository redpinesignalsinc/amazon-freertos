 ...
 *
 * @brief This files contains functions defination releated to QSPI.
 * 
 * @section Description
...
void qspi_write_to_flash(qspi_reg_t *qspi_reg,uint32_t len_in_bits,
		                        uint32_t cmd_addr_data,uint32_t cs_no)
{
	...
	qspi_reg->QSPI_MANUAL_CONFIG_REG = (
			((qspi_reg->QSPI_MANUAL_CONFIG_REG & ~0x6007) & ~TAKE_LEN_FRM_REG)
			| WRITE_TRIGGER | (cs_no << 13));
	// wait till QSPI becomes idle
	while(qspi_reg->QSPI_STATUS_REG & 1);
}

void qspi_switch_qspi2(
		qspi_reg_t *qspi_reg,
		uint32_t mode,
		uint32_t cs_no)
{
	...
	if(cs_no == CHIP_ZERO) {
		...
		qspi_reg->QSPI_BUS_MODE_REG = ((qspi_reg->QSPI_BUS_MODE_REG & ~0x6)
				| (mode << 1));
	} else  {
		// read the reg
...
uint32_t qspi_wait_flash_status_Idle(
		qspi_reg_t *qspi_reg,
...
void qspi_enable_status_reg_write(
		qspi_reg_t *qspi_reg,
		uint32_t flash_type,
		spi_config_t *spi_config,
		uint32_t cs_no)
    ...
    uint32_t wr_reg_delay_ms)
{
	...
	qspi_write_to_flash(qspi_reg, QSPI_16BIT_LEN,
			((reg_write_cmd << 8) | reg_write_value), cs_no);
...
void RSI_QSPI_ResetFlash(qspi_reg_t *qspi_reg, uint32_t cs_no) 
{
  ...
  ddr_mode = operating_mode & BIT(7);
	...
	if(!reset_type) {
     ...
     RSI_EGPIO_SetPinMux(EGPIO,0,13,0);
     RSI_EGPIO_SetPin(EGPIO,0,13,0);
     RSI_EGPIO_SetDir(EGPIO,0,13,0);
      if (operating_mode & BIT(4)) {
        qspi_usleep(150); 
      } else {
        qspi_usleep(50); 
      }
     RSI_EGPIO_SetPin(EGPIO,0,13,1);
    } else if(reset_type == 9) { 
     RSI_EGPIO_SetPinMux(EGPIO,0,14,0);
     RSI_EGPIO_SetPin(EGPIO,0,14,0);
     RSI_EGPIO_SetDir(EGPIO,0,14,0);
      if (operating_mode & BIT(4)) {
        qspi_usleep(150); 
      } else {
        qspi_usleep(50); 
      }
     RSI_EGPIO_SetPin(EGPIO,0,14,1);
...
void qspi_set_flash_mode(qspi_reg_t *qspi_reg,uint32_t data_mode,
		                       uint32_t cs_no,uint32_t ddr_mode_en,
		                        uint32_t flash_type)
{
		...
		uint32_t addr)
{
	...
	if(spi_config->spi_config_3._16bit_cmd_valid) {
		qspi_write_to_flash(qspi_reg, QSPI_16BIT_LEN,((spi_config->spi_config_1.read_cmd << 8) | spi_config->spi_config_3._16bit_rd_cmd_msb), cs_no);
	} else {
		qspi_write_to_flash(qspi_reg, QSPI_8BIT_LEN,
				(spi_config->spi_config_1.read_cmd | A8_BIT), cs_no);
	}
	...
	if(spi_config->spi_config_1.extra_byte_en) {
		...
		if(spi_config->spi_config_1.extra_byte_mode != prev_bus_mode) {
			qspi_switch_qspi2(qspi_reg, spi_config->spi_config_1.extra_byte_mode,
					cs_no);
...
void qspi_manual_read(qspi_reg_t *qspi_reg,spi_config_t *spi_config,
                      	uint32_t addr,uint8_t *data,uint32_t hsize,
		                    uint32_t len_in_bytes,uint32_t dma_flags,
		                    void *udmaHandle,void *gpdmaHandle)
{
	uint32_t loop_count;
  ...
  if(dma_flags & DEFAULT_DESC_MODE) {
	...
	if(spi_config->spi_config_2.swap_en == NO_SWAP) {
		...
		qspi_reg->QSPI_MANUAL_CONFIG_2_REG &=
				~(SWAP << (4 + spi_config->spi_config_2.cs_no));
	}
	if(hsize)
	{
  ...
  if(dma_flags & DEFAULT_DESC_MODE) { 
	...
	while(len_in_bytes){
		if(!once){
			...
			qspi_manual_config_reg =
					(qspi_reg->QSPI_MANUAL_CONFIG_REG & ~0xF8387FFF);
			qspi_manual_config_reg |= (spi_config->spi_config_2.cs_no << 13)
        		| (hsize << 19) | TAKE_LEN_FRM_REG;
		...
		qspi_reg->QSPI_MANUAL_CONFIG_REG = qspi_manual_config_reg
				| READ_TRIGGER  | ((loop_count & 0x3FF)<< 3) | (((loop_count >> 10) & 0x1F) << 27);

		// check whether dma mode is configured
    if (spi_config->spi_config_2.dma_mode) {
      if(dma_flags & DEFAULT_DESC_MODE) {
        // read using dma
        RSI_QSPI_GPDMA_ReadFromFifo((uint32_t)&(qspi_reg->QSPI_MANUAL_RD_WR_DATA_REG), (uint32_t)data, loop_count, ch_no);
        data += loop_count;
...
void qspi_auto_init(qspi_reg_t *qspi_reg,
		                    spi_config_t *spi_config)
{
	uint32_t offset;
	uint32_t *auto_1_ptr;
  ...
  uint32_t dummy_count = 0;
	...
	if (spi_config->spi_config_3._16bit_cmd_valid){
		auto_3_data |= QSPI_CMD_SIZE_16BIT_CSN0
				| (spi_config->spi_config_1.read_cmd << QSPI_RD_INST_CSN0_MSB);
	}else{
		auto_3_data &= ~QSPI_CMD_SIZE_16BIT_CSN0;
	}
 ...
 dummy_count = (spi_config->spi_config_1.no_of_dummy_bytes) | (spi_config->spi_config_4.no_of_ms_dummy_bytes << 4);
  ...
  if(spi_config->spi_config_2.dummy_cycles_for_controller) {
    ...
    dummy_count = ((dummy_count*8) + (spi_config->spi_config_2.dummy_cycles_for_controller * (1 << spi_config->spi_config_1.dummy_mode)));
  } else {
     auto_3_data &=  ~DUMMY_BYTE_OR_BIT_MODE;
  }
  ...
  auto_3_data |= (((dummy_count >> 4) & 0xF) << 1);
	...
	*auto_2_ptr |=
			(AUTO_ADDR_WIDTH) 																    //< address width 8, 9, 16 or 24 bit
	//	|  (spi_config->spi_config_2.jump_inst << 24)          //< cmd to be used in case of jump
			...
			| (0);         //< continuous read mode enable

	*auto_1_ptr =
		 ((dummy_count & 0xF) << 24)  //< no. of dummy bytes for read
	...
	if(spi_config->spi_config_2.wrap_len_in_bytes != NO_WRAP) {
		// send SET_BURST cmd to flash to initialize wrap
		qspi_write_to_flash(qspi_reg, 16,
...
void qspi_auto_read(uint32_t cs_no,uint32_t addr,uint8_t *data,
		                   uint32_t hsize,uint32_t len_in_bytes,
		                   spi_config_t *spi_config,uint32_t dma_flags)
{
	uint16_t *data_16bit;
	uint32_t *data_32bit;
	uint32_t pad;
...
void qspi_flash_init(qspi_reg_t *qspi_reg,
		                    spi_config_t *spi_config,uint32_t wr_reg_delay_ms)
{
	...
	opi_qpi = (spi_config->spi_config_1.inst_mode == OCTA_MODE)? BIT(3):((spi_config->spi_config_1.inst_mode == QUAD_MODE) ? BIT(2) : 0);
	flash_type = spi_config->spi_config_1.flash_type;
  ...
  // Store flash operating mode and reset type in battery backup flipflops.
	...
	case AT_QUAD_FLASH:
		...
		if(is_quad_mode) {
			qspi_flash_reg_write(qspi_reg, WCON, ATMEL_QEN,
					spi_config->spi_config_2.cs_no, wr_reg_delay_ms);
		}
#if 0      
		if(spi_config->spi_config_2.protection) {
			// Removes write protection, so that memory can be written
	...
	case MX_QUAD_FLASH:
        ...
        if((is_quad_mode) || (spi_config->spi_config_4.prot_top_bottom)
            || (spi_config->spi_config_4.qspi_ddr_clk_en)) {
      ...
      if (wr_status_reg) {
		...
		if (spi_config->spi_config_1.inst_mode == QUAD_MODE) {
			...
			qspi_set_flash_mode(qspi_reg, spi_config->spi_config_1.inst_mode,
					spi_config->spi_config_2.cs_no, 0, flash_type);
		}
		break;
    ...
    if(spi_config->spi_config_4.prot_top_bottom) {
      ...
      if(flash_status2 & BIT(3)) {
		...
		qspi_status_control_reg_write(spi_config, qspi_reg, WCFG2, 0x300, dummy_clks, spi_config->spi_config_2.cs_no, wr_reg_delay_ms);
		qspi_status_control_reg_write(spi_config, qspi_reg, WCFG2, 0, str_dtr, spi_config->spi_config_2.cs_no, wr_reg_delay_ms);
		break;
	...
	case MICRON_QUAD_FLASH:
		...
		qspi_flash_reg_write(qspi_reg, WR_VOL_CON_REG,

				((dummy_clks << 4) |
						xip_mode | spi_config->spi_config_2.wrap_len_in_bytes),
						spi_config->spi_config_2.cs_no, wr_reg_delay_ms);
		...
		if(reg_cfg) {
			 ...
			 */
			qspi_flash_reg_write(qspi_reg, WR_ENHN_VOL_CON_REG,
					reg_cfg, spi_config->spi_config_2.cs_no, wr_reg_delay_ms);
		}
		break;
	...
	//		spi_config->spi_config_2.cs_no, 0, flash_type);
		...
		qspi_status_control_reg_write(spi_config, qspi_reg, STS_CTRL, 3,
				(dummy_clks | (spi_config->spi_config_2.wrap_len_in_bytes << 5)), spi_config->spi_config_2.cs_no, wr_reg_delay_ms);
		// enabling opi or qpi and str or dtr
		if(opi_qpi || str_dtr) {
			qspi_flash_reg_write(qspi_reg,STS_BYT2,
					(opi_qpi | str_dtr), spi_config->spi_config_2.cs_no, wr_reg_delay_ms);
		}
		break;
...
void RSI_QSPI_ConfigQspiDll(spi_config_t *spi_config, qspi_reg_t *qspi_reg)
{
	...
	qspi_reg->QSPI_CLK_CONFIG_REG = (QSPI_DLL_CALIB |QSPI_DLL_TX_EN | QSPI_DLL_RX_EN | (0x6 << 22)
			| (0x6 << 12) | BIT(18) | (spi_config->spi_config_2.qspi_clk_en << 8));  // Octa_mode_enable_with_dqs
	//wait till 10 clocks
	while(delay) {
		__ASM("nop");
		delay--;
...
void qspi_spi_init(qspi_reg_t *qspi_reg,spi_config_t *spi_config,
		                  uint32_t flash_init_req,uint32_t wr_reg_delay_ms,
		                   uint8_t fifo_thrsld)
{
	...
	qspi_reg->QSPI_BUS_MODE_REG =
			((qspi_reg->QSPI_BUS_MODE_REG & (MASK_D3_D2(dataline_pos)))
					|  (spi_config->spi_config_1.d3d2_data << dataline_pos)
					|  (spi_config->spi_config_2.neg_edge_sampling << 16));

	qspi_reg->OCTA_SPI_BUS_CONTROLLER =
			((qspi_reg->OCTA_SPI_BUS_CONTROLLER & (MASK_D7_D4(d7_d4_data_pos)))
	...
	qspi_switch_qspi2(qspi_reg,  spi_config->spi_config_1.inst_mode,
			spi_config->spi_config_2.cs_no);
  ...
  if (spi_config->spi_config_4.auto_csn_based_addr_en) {
	...
	// Enable HW_CTRL_MODE
	qspi_reg->QSPI_MANUAL_CONFIG_REG |=  (spi_config->spi_config_2.cs_no << 13)
    		| HW_CTRL_MODE;
...
// configure QSPI fifo thresholds
	qspi_reg->QSPI_FIFO_THRESHOLD_REG =
			(QSPI_FIFO_AFULL_TH << 4) | QSPI_FIFO_AEMPTY_TH;
	}
...
void qspi_spi_erase(qspi_reg_t *qspi_reg,spi_config_t *spi_config,
		                   uint32_t erase_cmd,uint32_t blk_sec_addr,
		                   uint32_t dis_hw_ctrl,uint32_t wr_reg_delay_ms)
{
	uint32_t cs_no;
		...
		void *gpdmaHandle)
{
	...
	volatile uint32_t auto_mode_address;
	uint8_t *data_in;
  uint32_t ch_no;
	status = RSI_OK;
	once = 1;
  ...
  dis_hw_ctrl &= ~BIT(31);
	...
	while(len_in_bytes) {
		...
		if(spi_config->spi_config_3.ddr_mode_en) {
			if(flash_type == MX_OCTA_FLASH) {
				qspi_write_to_flash(qspi_reg, QSPI_16BIT_LEN, ((spi_config->spi_config_3.wr_cmd << 8) | spi_config->spi_config_4._16bit_wr_cmd_msb), cs_no);
			} else {
	...
	if(check_en) {
		...
		len_in_loop = 0;
		while(length >= 4){
			if(*(volatile uint32_t *)(auto_mode_address + address + len_in_loop) == *(uint32_t *)((uint32_t)data_in + len_in_loop)) {
				len_in_loop += 4;
				length -= 4;
			} else {
		...
		while (length > 0) {
			if(*(volatile uint8_t *)(auto_mode_address + address + len_in_loop) == *(uint8_t *)((uint32_t)data_in + len_in_loop)) {
				len_in_loop += 1;
				length -= 1;
			} else {
				status = (uint32_t)RSI_FAIL;
...
void RSI_QSPI_TIMER_Config(void)
{
  ...
  // FIXME , Option to configure from mbr
	timers_microsec_timer_config(TIMERS, TIMER_0, 80, 0 ,MICRO_SEC_MODE);
}
