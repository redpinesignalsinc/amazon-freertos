 ...
 *
 * @brief This files contains functions prototypes related to sd memory peripheral
 *
 * @section Description
 * This file contains the list of function prototypes for the sd memory and low level function definitions
...
error_t RSI_SDMEM_SendCsdCmd9(SMIH_CARD_CONFIG_T *pSmihConfig)
{
  ...
  data.data             = NULL;
	...
	if (pSmihConfig->cardType ==SMIH_CARD_STANDARD )
	{
		...
		u32Csize = ((unsigned int)pSdCsd->deviceSize3 << 10)
    + ((unsigned int)pSdCsd->deviceSize2 << 2)
    + pSdCsd->deviceSize1;
	...
	{
		/* high capacity card */
		u32Csize = ((unsigned int)pSdhcCsd->deviceSize3 << 16)
    + ((unsigned int)pSdhcCsd->deviceSize2 << 8)
    + pSdhcCsd->deviceSize1;
 ...
 * @param[in]    pSmihConfig            pointder to the sd card config structure
 * @param[in]    DataIn                 Data buffer to send
 * @param[in]    BlockIndex             block index value
 * @param[in]    Count                  blocks count value
 * @return       RSI_OK                 If the command sent successfully
 *               ERROR_TIMEOUT          If the command error timeout occures.
 */
error_t RSI_SDMEM_BlocksWrite(SMIH_CARD_CONFIG_T *pSmihConfig,
		const uint8_t *DataIn,uint32_t BlockIndex,uint32_t BlockCount)
{
	SMIH_TRANSFER_T transfer = { 0 };
	SMIH_CMD_FEILD_T cmd  = { 0 };
	SMIH_DATA_FEILD_T data = { 0 };
	...
	else
	{
		...
		if(((pSmihConfig->maxSectorNum - BlockIndex) < BlockCount) || (pSmihConfig->maxSectorNum < (BlockIndex + BlockCount)))
		{
			return INVALID_PARAMETERS;
		}
		data.data       = DataIn;
 ...
 */
error_t RSI_SDMEM_BlocksRead(SMIH_CARD_CONFIG_T *pSmihConfig,uint8_t *DataOut,uint32_t BlockIndex,uint32_t BlockCount)
{
	SMIH_TRANSFER_T transfer = { 0 };
	SMIH_CMD_FEILD_T cmd = { 0 };
	SMIH_DATA_FEILD_T data = { 0 };
...
error_t RSI_SDMMC_SendOperationConditionCmd1(SMIH_CARD_CONFIG_T *pSmihConfig,uint32_t Arg)
{
	...
	else if(transfer.command->response[0] & BIT(30))
	{
		pSmihConfig->cardType =  SMIH_CARD_HIGH_CAPACITY;
	}
	return RSI_OK;
