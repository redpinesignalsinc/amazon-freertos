 ...
 *
 * @brief This files contains functions related to SIO
 *
 * @section Description
 * This file contains the list of functions for configuring the SIO.
...
{
	clk_peripheral_clk_enable3(M4CLK , (SGPIO_PCLK_ENABLE ));
	return ;
}
...
void IRQ037_Handler(void) // with interrupt
{
	...
	if(gstcSioCb.i2s_sio.u8I2sValid)
	{
		...
		if(u32SwapStsReg & (1 << gstcSioCb.i2s_sio.u8I2sDataInCh))
		{
			...
			if(gstcSioCb.i2s_sio.pstcI2sXfer->rxDoneCount < gstcSioCb.i2s_sio.pstcI2sXfer->rxCount){
				/*Fill the buffer */
				((uint32_t *)gstcSioCb.i2s_sio.pstcI2sXfer->rxBuff)[gstcSioCb.i2s_sio.pstcI2sXfer->rxDoneCount] = pstcSio->SIO_BUFFER_REG[gstcSioCb.i2s_sio.u8I2sDataInCh];
		...
		if(u32SwapStsReg & (1 << gstcSioCb.i2s_sio.u8I2sDataOutCh))
		{
			...
			if(gstcSioCb.i2s_sio.pstcI2sXfer->txDoneCount < gstcSioCb.i2s_sio.pstcI2sXfer->txCount){
				pstcSio->SIO_BUFFER_REG[gstcSioCb.i2s_sio.u8I2sDataOutCh]= ((uint32_t *)gstcSioCb.i2s_sio.pstcI2sXfer->txBuff)[gstcSioCb.i2s_sio.pstcI2sXfer->txDoneCount];
		...
		if(u32SwapStsReg & (1 << gstcSioCb.spi_sio.u8SpiMosiCh))
		{
			...
			if(gstcSioCb.spi_sio.pstscSpiXfer->txDoneCount < gstcSioCb.spi_sio.pstscSpiXfer->txCount){
				...
				if((pstcSio->SIO_SHIFT_COUNT_PRELOAD_REG_b[gstcSioCb.spi_sio.u8SpiMisoCh].REVERSE_LOAD == 1)
					...
					/*MSB First*/
					pstcSio->SIO_BUFFER_REG[gstcSioCb.spi_sio.u8SpiMosiCh] = (u32Data << (32 - gstcSioCb.spi_sio.pstscSpiXfer->u8BitLen));
				}
				else{
					/*LSB First*/
					pstcSio->SIO_BUFFER_REG[gstcSioCb.spi_sio.u8SpiMosiCh] = (u32Data >> (32 - gstcSioCb.spi_sio.pstscSpiXfer->u8BitLen));
				...
				/*Trigger TX done call back*/
				pstcSio->SIO_SWAP_INTR_EN_CLEAR_REG = ((1 << gstcSioCb.spi_sio.u8SpiMosiCh | (1 << gstcSioCb.spi_sio.u8SpiMisoCh)));
			...
			if(gstcSioCb.spi_sio.pstscSpiXfer->u8Status == SioSpiBusy){
				...
				pstcSio->SIO_ENABLE_REG_b.SIO_OPERATION_ENABLE |= ((1 << gstcSioCb.spi_sio.u8SpiMisoCh) |
						(1 << gstcSioCb.spi_sio.u8SpiMosiCh)   |
						(1 << gstcSioCb.spi_sio.u8SpiClkCh));

				pstcSio->SIO_PAUSE_REG_b.SIO_POSITION_COUNTER_DISABLE |= ((1 << gstcSioCb.spi_sio.u8SpiMisoCh)   |
						(1 << gstcSioCb.spi_sio.u8SpiMosiCh) |
						(1 << gstcSioCb.spi_sio.u8SpiClkCh));
			}
		}
	...
	if(gstcSioCb.uart_sio.u8UartValid)
	{
		...
		{
			pstcSio->SIO_DATA_POS_COUNT_REG_b[gstcSioCb.uart_sio.u8UartRxCh].RELOAD_VALUE = ((gstcSioCb.uart_sio.pstcSioUart->u8Bitlen + gstcSioCb.uart_sio.pstcSioUart->u8StopBits +gstcSioCb.uart_sio.u8ParityEn +1 )-1);
		...
		if((pstcSio->SIO_SWAP_INTR_STATUS_REG) & (1 << gstcSioCb.uart_sio.u8UartRxCh))
		{
			if(gstcSioCb.uart_sio.u16UartRxDone < gstcSioCb.uart_sio.u16UartRxCnt)
			{
				...
				u32Data = (u32Data >> (32 - (gstcSioCb.uart_sio.pstcSioUart->u8Bitlen + gstcSioCb.uart_sio.pstcSioUart->u8StopBits +gstcSioCb.uart_sio.u8ParityEn +1 )));
		...
		if((u32SwapStsReg) & (1 << gstcSioCb.uart_sio.u8UartTxCh))
		{
			...
			if(gstcSioCb.uart_sio.u16UartTxDone < gstcSioCb.uart_sio.u16UartTxCnt){
				...
				u16UartDataFrame = RSI_SIO_UartFramePrepare(gstcSioCb.uart_sio.pstcSioUart->u8StopBits ,
						u32Data,
						gstcSioCb.uart_sio.pstcSioUart->u8Parity , gstcSioCb.uart_sio.pstcSioUart->u8Bitlen);
...
error_t RSI_SIO_SpiTrasnfer(volatile SIO_Type *pstcSio , stc_sio_spi_xfer_t  *pstcSpiXfer)
{
	...
	/*Enable and pause the shift counters */
	pstcSio->SIO_ENABLE_REG_b.SIO_OPERATION_ENABLE = ((0x1 << gstcSioCb.spi_sio.u8SpiMisoCh) |
			(1 << gstcSioCb.spi_sio.u8SpiMosiCh)   |
			(1 << gstcSioCb.spi_sio.u8SpiClkCh));

	pstcSio->SIO_PAUSE_REG_b.SIO_POSITION_COUNTER_DISABLE = ((1 << gstcSioCb.spi_sio.u8SpiMisoCh)   |
			(0x1 << gstcSioCb.spi_sio.u8SpiMosiCh) |
			(1 << gstcSioCb.spi_sio.u8SpiClkCh));
...
error_t RSI_SIO_InitI2s(volatile SIO_Type *pstcSio , stc_sio_i2s_config_t  *pstcI2sConfig)
{
	...
	/*Enable Swap interrupts for MOSI and MISO channels */
	pstcSio->SIO_SWAP_INTR_EN_SET_REG |= (1 << pstcI2sConfig->u8SioI2sDataOutChannel) | (1 << pstcI2sConfig->u8SioI2sDataInChannel);
	/*unmask interrupts*/
	pstcSio->SIO_SWAP_INTR_MASK_CLEAR_REG |= (1 << pstcI2sConfig->u8SioI2sDataOutChannel) | (1 << pstcI2sConfig->u8SioI2sDataInChannel);
...
error_t RSI_SIO_I2sTrasnfer(volatile SIO_Type *pstcSio , stc_sio_i2s_xfer_t  *pstcI2sXfer)
{
	...
	pstcSio->SIO_SWAP_INTR_EN_SET_REG |= (1 << gstcSioCb.i2s_sio.u8I2sDataInCh) | ((1 << gstcSioCb.i2s_sio.u8I2sDataOutCh));
	...
	pstcSio->SIO_BUFFER_REG[gstcSioCb.i2s_sio.u8I2sDataOutCh] = ((uint32_t *)pstcI2sXfer->txBuff)[pstcI2sXfer->txDoneCount];
	pstcI2sXfer->txDoneCount++;

	/*Enable and pause the shift counters  */ /* enable required channel function */
	pstcSio->SIO_ENABLE_REG_b.SIO_OPERATION_ENABLE |= (1 << gstcSioCb.i2s_sio.u8I2sDataOutCh ) |
			(1 << gstcSioCb.i2s_sio.u8I2sWsCh      ) |
			(1 << gstcSioCb.i2s_sio.u8I2sClkCh     ) |
 ...
 */
uint16_t RSI_SIO_UartFramePrepare(uint8_t u8StopBits  , uint16_t u16Data , uint8_t u8ParitySet ,
		uint8_t u8DataLen)
{
	uint16_t u16Frame = 0;
	uint8_t u8Parity =0 ;
 ...
 */
uint16_t RSI_SIO_UartFrameExtract(uint8_t u8StopBits  , uint16_t u16UartDataFrame ,
		uint8_t u8ParitySet , uint8_t u8DataLen)
{
	uint16_t u16Data =0;
	uint8_t u8Parity =0;
	if(u8ParitySet == 1)
...
uint8_t RSI_SIO_UartInit(SIO_Type *pstcSio , stc_sio_uart_config_t *pstcConfig)
{
	...
	pstcSio->SIO_DATA_POS_COUNT_REG_b[pstcConfig->u8SioUartTxChannel].RELOAD_VALUE = ((pstcConfig->u8Bitlen + pstcConfig->u8StopBits + gstcSioCb.uart_sio.u8ParityEn + 10)-1);
	return RSI_OK;
}
 ...
 * @return       return \ref RSI_OK on success and on failure return error code.
 */
error_t RSI_SIO_I2cTransfer(volatile SIO_Type *pstcSio , stc_sio_i2c_config_t *pstcConfig ,
		uint8_t u8SlaveAddr , uint8_t *u8PtrTxDat , uint16_t  u16TxLen ,
		uint8_t *u8PtrRxDat , uint16_t  u16RxLen  )
{
	uint32_t u32Enable = 0;
