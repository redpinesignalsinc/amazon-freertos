 ...
 *
 * @brief This files contains functions prototypes related to SMIH peripheral
 *
 * @section Description
 * This file contains the list of function prototypes for the SMIH and low level function definitions
 ...
 *               MemType               : typpe of operation to be pragram
 *                                     1 for memory operations ,o for IO operations
 ...
 * @param[in]    Enable4BitMode       bus width mode
 ...
 * @return       RSI_OK             If stops data transfer.
 */
 ...
 * @param[in]    pSdiohInternData   Pointer to the smih internal data.
 */
 ...
 * @brief        This API is used to initialize the smih command configuration structure.
 ...
 * @return       RSI_OK      If new frwquency is set
 */
...
error_t smih_check_for_error_interrupt(void)
{
	...
	if (event.commandTimeoutError || event.commandCrcError || event.commandEndBitError || event.commandIndexError || event.dataTimeoutError|| event.dataEndbitError || event.dataCrcError)
	{
...
/**
 * @fn           error_t smih_send_data(SMIH_CMD_FEILD_T *pCommand)
 * @brief        This API is used to write the data to the Smih Fifo
 * @param[in]    pCommand              Pointer to the command data structure
...
error_t smih_send_data(SMIH_TRANSFER_T *pTransfer)
{
	...
	{
		while(event.bufferWriteReady == 0);
    ...
    pBuffer = (uint32_t *)pTransfer->data->data;
		...
		while(event.transferComplete == 0);
...
/**
 * @fn           error_t smih_receive_data(SMIH_CMD_FEILD_T *pCommand)
 ...
 * @brief        This API is used to send command to the SDIO slave.
 * @param[in]    pTransfer            Pointer to the command and data structure
 * @return       ERROR_SMIH           If Parameter is invalid.
 *               ERROR_TIMEOUT        If the command error timeout occures.
 */
