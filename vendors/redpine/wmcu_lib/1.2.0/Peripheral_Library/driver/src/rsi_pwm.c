 ...
 *
 * @brief This files contains functions defination releated to PWM peripheral
 * 
 * @section Description
...
{	
	if((pMCPWMDeadTimeConfig->counterSelect == COUNTER_A)||
		 (pMCPWMDeadTimeConfig->counterSelect == COUNTER_B))
	{
		...
		{		
			case PWM_CHNL_0:
			{
				if(pMCPWMDeadTimeConfig->counterSelect == COUNTER_A)
				{
					if((pMCPWMDeadTimeConfig->preScaleA >= 0)&&
             (pMCPWMDeadTimeConfig->preScaleA <= 3))
          {
					...
					else
          {
				...
				{
					if((pMCPWMDeadTimeConfig ->preScaleB >= 0)&&
             (pMCPWMDeadTimeConfig->preScaleB <= 3))
          {
                                       ...
                                       uint8_t chnlNum )
{
	...
	switch(chnlNum)
  {
    ...
    case PWM_CHNL_1:
      ...
      pMCPWM ->PWM_TIME_PRD_PARAM_REG_CH1_b.PWM_TIME_PRD_POST_SCALAR_VALUE_CH1 = preScale;       
      break;
    case PWM_CHNL_2:
      pMCPWM ->PWM_TIME_PRD_PARAM_REG_CH2_b.PWM_TIME_PRD_POST_SCALAR_VALUE_CH2 = postScale; 
      pMCPWM ->PWM_TIME_PRD_PARAM_REG_CH2_b.PWM_TIME_PRD_POST_SCALAR_VALUE_CH2 = preScale;        
      break;
    case PWM_CHNL_3:
      pMCPWM ->PWM_TIME_PRD_PARAM_REG_CH3_b.PWM_TIME_PRD_POST_SCALAR_VALUE_CH3 = postScale; 
      pMCPWM ->PWM_TIME_PRD_PARAM_REG_CH3_b.PWM_TIME_PRD_POST_SCALAR_VALUE_CH3 = preScale;      
      break;
		default:
      return ERROR_PWM_INVALID_CHNLNUM;
  } 
