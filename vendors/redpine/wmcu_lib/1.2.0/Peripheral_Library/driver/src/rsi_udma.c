 ...
 *
 * @brief This files contains functions defination releated to UDMA peripheral
 * 
 * @section Description
 ...
 */
static error_t RSI_UDMA_ChannelControlsDisable(RSI_UDMA_HANDLE_T pHandle, RSI_UDMA_CHA_CFG_T *pCfg)
{
...
error_t udma_setup_channel_transfer( RSI_UDMA_HANDLE_T pHandle, RSI_UDMA_CHA_CFG_T *pCfg,
...
{	
  ...
  RSI_UDMA_DATACONTEXT_T *pDrv = (RSI_UDMA_DATACONTEXT_T *) pHandle;
	...
	if((channelTableIndex & 0xffff) < 64)
	{
			...
			/* Set transfer mode */
			if((vsUdmaChaConfigData.transferType == UDMA_MODE_BASIC)||
				 (vsUdmaChaConfigData.transferType == UDMA_MODE_AUTO)||
         (vsUdmaChaConfigData.transferType == UDMA_MODE_PINGPONG))
			...
			{
				pUDMAChaCtrlDataStruct[channelTableIndex].vsUDMAChaConfigData1.totalNumOfDMATrans = vsUdmaChaConfigData.totalNumOfDMATrans;
			}
			else
			{
		  ...
		  /* Set arbitration */
			...
			/* set source size  */
			if((vsUdmaChaConfigData.srcSize == SRC_SIZE_8)||
				 (vsUdmaChaConfigData.srcSize == SRC_SIZE_16)||
         (vsUdmaChaConfigData.srcSize == SRC_SIZE_32))			   
			...
			if((vsUdmaChaConfigData.srcInc == SRC_INC_8)||
				 (vsUdmaChaConfigData.srcInc == SRC_INC_16)||
         (vsUdmaChaConfigData.srcInc == SRC_INC_32)||
			   (vsUdmaChaConfigData.srcInc == SRC_INC_NONE))  
			{
			...
			if((vsUdmaChaConfigData.dstSize == DST_SIZE_8)||
				 (vsUdmaChaConfigData.dstSize == DST_SIZE_16)||
         (vsUdmaChaConfigData.dstSize == DST_SIZE_32))
		  ...
		  if((vsUdmaChaConfigData.dstInc == DST_INC_8)||
				 (vsUdmaChaConfigData.dstInc == DST_INC_16)||
...
error_t udma_set_channel_scatter_gather_transfer( RSI_UDMA_HANDLE_T pHandle, uint8_t dmaCh, uint32_t taskCount,
...
uint32_t udma_get_channel_transfer_length(RSI_UDMA_HANDLE_T pHandle, RSI_UDMA_CHA_CFG_T *pCfg,
																					RSI_UDMA_CHA_CONFIG_DATA_T vsUDMAChaConfigData)
{		
 ...
 * @return 		  none  
 */
static void RSI_UDMA_SetSingleRequest(RSI_UDMA_HANDLE_T pHandle)
{
  RSI_UDMA_DATACONTEXT_T *pDrv = (RSI_UDMA_DATACONTEXT_T *) pHandle;
...
/**
 * @fn          static void RSI_UDMA_AckEnable(RSI_UDMA_HANDLE_T pHandle,uint32_t peripheral)
 ...
 * @return 		  none
 */
static void RSI_UDMA_AckEnable(RSI_UDMA_HANDLE_T pHandle,uint32_t peripheral)
{
	switch(peripheral)
	{
		case USART0_ACK:
