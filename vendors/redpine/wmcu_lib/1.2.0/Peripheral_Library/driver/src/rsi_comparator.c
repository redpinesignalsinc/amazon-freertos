 ...
 *
 * @brief This files contains functions prototypes related to COMPARATOR.
 * 
 * @section Description
 * This file contains the list of function defination for the COMPARATOR peripheral 
 * with different configuring parameters.
 * 
 *
 */
#include "rsi_chip.h"
                        ...
                        uint8_t    sel_n_mux , uint8_t hyst_en     , uint8_t filter_en)
{
  ...
  if((comp_number > MAX_NUM_OF_COMPARATORS)||(comp_number < MIN_NUM_OF_COMPARATORS) || (sel_p_mux > MAX_P_SEL_MUX)
	...
	if(((sel_p_mux == 3) || (sel_n_mux == 3))||
		 ((sel_p_mux == 4) || (sel_n_mux == 4)))
	{
 ...
 */
error_t RSI_COMP_Enable(AUX_ADC_DAC_COMP_Type* comp,uint8_t comp_number , 
                        uint8_t enable)
{
	uint32_t i = 0;
...
error_t RSI_COMP_ResBank(AUX_ADC_DAC_COMP_Type* comp,uint16_t value_thrsh)
{
  if((value_thrsh>MAX_THRSHOLD_VALUE))
  {
	...
	if((comp->COMPARATOR1_b.CMP1_MUX_SEL_P == 5) 
		  || (comp->COMPARATOR1_b.CMP1_MUX_SEL_N == 5))
  {
	...
	if((comp->COMPARATOR1_b.CMP2_MUX_SEL_P == 5) 
		  || (comp->COMPARATOR1_b.CMP2_MUX_SEL_N == 5))
  {
...
error_t RSI_COMP_ReferenceScaler(AUX_ADC_DAC_COMP_Type* comp,uint16_t scalar_factor_value)
{
  if((scalar_factor_value>MAX_SCALAR_FACTOR))
  {
	...
	if((comp->COMPARATOR1_b.CMP1_MUX_SEL_P == 4) 
		  || (comp->COMPARATOR1_b.CMP1_MUX_SEL_N == 4))
  {
	...
	if((comp->COMPARATOR1_b.CMP2_MUX_SEL_P == 4) 
		  || (comp->COMPARATOR1_b.CMP2_MUX_SEL_N == 4))
  {
...
{
  if((comp_number > MAX_NUM_OF_COMPARATORS)||
		 (comp_number < MIN_NUM_OF_COMPARATORS))
  {
		return INVALID_PARAMETERS;
  }		
