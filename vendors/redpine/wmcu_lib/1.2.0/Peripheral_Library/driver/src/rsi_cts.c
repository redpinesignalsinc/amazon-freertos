...
/* ULP Memory address range to retain CTS information in sleep of core 
   start address 0x24061000 
   End address 0x24061800	 
   */
...
static const CTS_PIN_Config cts_pins[] ={
	{SENSOR1,AGPIO_PIN8,SENSOR1_ENABLE},{SENSOR2,AGPIO_PIN9,SENSOR2_ENABLE},{SENSOR3,AGPIO_PIN10,SENSOR3_ENABLE},		
	{SENSOR4,AGPIO_PIN7,SENSOR4_ENABLE},{SENSOR5,AGPIO_PIN6,SENSOR5_ENABLE},{SENSOR6,AGPIO_PIN3,SENSOR6_ENABLE},
 ...
 *              
 */
void RSI_CTS_ConfigRefVoltage(CTS_Type *cts,int16_t ref_voltage,boolean_t enable,
		uint16_t *samples, uint16_t *avg_samples)
{
	...
	{	
				...
				&& (Threshold  < ON_TIME*AVG_MULTIPLIER))
		{
			...
			{
				Threshold=0; avg_count=0; sample_count=0;
				if(default_cal != 4)
				{	
					default_cal=2;
				}
		...
		{		
			...
			calib=1; avg_count=0;  sample_count=0;                     //ref_voltage changes switches to fast calibration
			default_cal=2;
			memset(avg_samples,0,(NO_OF_SLOW_SAMPLES *2));    
			memset(samples,0,(NO_OF_SAMPLES * 2));
		}
