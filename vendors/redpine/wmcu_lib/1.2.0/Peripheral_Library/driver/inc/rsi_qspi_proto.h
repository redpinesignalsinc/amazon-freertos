...
#define PADSELECTION                (* (volatile uint32_t *) (0x41300000+0x610))  /*!< PAD selection (0 t0 21) A value of 1 on this gives control to M4SS */
...
void (*RSI_QSPI_ConfigureQspiRead)(spi_config_t *spi_config, qspi_func_t *qspi_func);
	...
	void (*qspi_qspiload_key)(qspi_reg_t *qspi_reg,
			uint32_t *key,
      uint32_t kh_enable);
	void (*qspi_qspiload_nonce)(qspi_reg_t *qspi_reg,
			uint32_t *nonce);
	void (*qspi_seg_sec_en)(qspi_reg_t *qspi_reg,
			uint32_t seg_no,
			uint32_t start_addr,
			uint32_t end_addr);
	...
	void (*qspi_flash_protection)(spi_config_t *spi_config,
	 ...
	 void (*RSI_QSPI_ConfigQspiDll)(spi_config_t *spi_config,
      qspi_reg_t *qspi_reg);
	  
	  void (*RSI_QSPI_ResetFlash)(qspi_reg_t *qspi_reg,
       uint32_t cs_no);

      void (*RSI_QSPI_UpdateOperatingMode_and_ResetType)(qspi_reg_t *qspi_reg,
...
void qspi_qspiload_key(
		qspi_reg_t *qspi_reg,
		uint32_t *key,
		uint32_t kh_enable);

void qspi_qspiload_nonce(
		qspi_reg_t *qspi_reg,
		uint32_t *nonce);

void qspi_seg_sec_en(
		qspi_reg_t *qspi_reg,
		uint32_t seg_no,
		uint32_t start_addr,
		uint32_t end_addr);
