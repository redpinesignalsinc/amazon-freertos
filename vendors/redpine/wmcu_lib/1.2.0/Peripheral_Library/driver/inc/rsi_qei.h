...
#define QEI_POS_CNT_ERR_INTR_LVL	    BIT(2) /*!< Whenever number of possible positions are mismatched with actual positions are received between two index pulses this will raised. */
...
#define QEI_POS_CNT_MAT_INTR_LVL			BIT(4) /*!< This is raised when the position counter reaches position match value, which is programmable */
...
#define QEI_SWAP_PHASE_AB_B  		    BIT(1)  /*!< Phase A and Phase B Input Swap Select bit 1 = Phase A and Phase B inputs are swapped*/
#define QEI_POS_CNT_RST_WITH_IDX_EN  	    BIT(2)  /*!< 1 = position counter is getting reset for every index pulse 0 = position counter is getting*/
...
#define QEI_DIGITAL_FILTER_BYPASS	    BIT(9)  /*!< 1= digital filter is bypassed for all input signals (position A, position B and Index) */
#define QEI_TIMER_MODE			    BIT(10) /*!< 1 = timer mode. In this mode, decoded timer pulse and direction are taken from position A and position B pins respectively. */
