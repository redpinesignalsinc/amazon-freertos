 ...
 *
 * @brief This files contains functions prototypes releated to QSPI.
 * 
 * @section Description
 * This file contains the list of function prototypes for the QSPI and configuring parameters
...
#define GET_POS                      ((spi_config->spi_config_2.cs_no == CHIP_THREE) ? 30:         \
                                      (spi_config->spi_config_2.cs_no == CHIP_TWO  ) ? 26:         \
                                      (spi_config->spi_config_2.cs_no == CHIP_ONE  ) ? 14:         \
                                                                                       10) 

#define GET_POS_D7_D4                ((spi_config->spi_config_2.cs_no == CHIP_THREE) ? 24:         \
...
#define READ_4M_FLASH(_NUM_BYTES, _CS_NO, Hsize)                                                            \
    qspi_reg->QSPI_MANUAL_CONFIG_REG = (qspi_reg->QSPI_MANUAL_CONFIG_REG & ~0xF8387FFF)                \
      | READ_TRIGGER | (_CS_NO << 13)  | ((_NUM_BYTES & 0x3FF)<< 3) | (((_NUM_BYTES >> 10) & 0x1F) << 27)   \
...
#define QSPI_MANUAL_BUS_SIZE(_CHIP_SELECT)                                                           \
  (_CHIP_SELECT == CHIP_ZERO) ?                                                                      \
      ((qspi_reg->QSPI_BUS_MODE_REG & 0x6) >> 1)                                                     \
      : ((qspi_reg->QSPI_MANUAL_CONFIG_2_REG >> (8 + ((_CHIP_SELECT - 1) * 2))) & 0x3)

#define QSPI_CMD_DDR_MODE          (spi_config->spi_config_2.cs_no ? (qspi_reg->QSPI_AUTO_CONFIG3_CSN1 & BIT(DDR_CMD)) : (qspi_reg->QSPI_AUTO_CONFIG3 & BIT(DDR_CMD)))
#define QSPI_DATA_DDR_MODE          (spi_config->spi_config_2.cs_no ? (qspi_reg->QSPI_AUTO_CONFIG3_CSN1 & BIT(DDR_CMD)) : (qspi_reg->QSPI_AUTO_CONFIG3 & BIT(DDR_DATA)))
