...
def check_platform_name(root, vendor, board):
    ...
    freertos_config_tests_path = \
        os.path.join(root, 'vendors', vendor, 'boards' , board, 'aws_tests/config_files/FreeRTOSConfig.h')
    freertos_config_demos_path = \
        os.path.join(root, 'vendors', vendor, 'boards' , board, 'aws_demos/config_files/FreeRTOSConfig.h')
...
PORTABLE_LAYER_PATHS = [
    'vendors/{vendor}/boards/{board}/ports/pkcs11/aws_pkcs11_pal.c',
    'vendors/{vendor}/boards/{board}/ports/wifi/aws_wifi.c',
    'vendors/{vendor}/boards/{board}/ports/ota/aws_ota_pal.c',
    'vendors/{vendor}/boards/{board}/ports/secure_sockets/aws_secure_sockets.c'
]
...
def check_license(root, vendor, board):
    ...
    config_files_tests_path = os.path.join(
        root, 'vendors/{vendor}/boards/{board}/aws_tests/config_files'.format(vendor=vendor, board=board)
    )
    config_files_demos_path = os.path.join(
        root, 'vendors/{vendor}/boards/{board}/aws_demos/config_files'.format(vendor=vendor, board=board)
    )
...
def check_eclipse(root, project_path, vendor, board, ide):
    ...
    errors = []
    root_var_set = set()
    demo_project_path = os.path.join(root, 'projects', vendor, board, ide, project_path, '.project')
    demo_cproject_path = os.path.join(root, 'projects', vendor, board, ide, project_path, '.cproject')
